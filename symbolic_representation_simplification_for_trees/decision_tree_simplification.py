# This script trains a decision tree on input data, translates it into SMT formulas using two (can be more) methods:
    # as nested z3.If;
    # as flatten z3.Implies;
# And compare their running times on the GearOpt_BO algorithm.
...
def build_decision_tree(X_train, y_train, depth=30):
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        sys.stderr.write("Usage: python3 decision_tree_simplification.py [specifications (.json)] [data (.csv)]\n")
        sys.exit(1)
        ...
        # next, encode tree model into z3 in two ways
        z3_nested_if_else = copy_solver(z3_solver)
        z3_flatten_imply = copy_solver(z3_solver)

        # properties of the tree to be used
        ...
        # 3.1
        z3_nested_if_else.add(z3.Real('y') == as_nested_if_then_else(z3_features, 0, features, thresholds, values, children_left, children_right))

        # 3.2
        all_paths = []
        as_flatten_implications(all_paths, [], z3_features, 0, features, thresholds, values, children_left, children_right)
        
        for path in all_paths:
            z3_flatten_imply.add(path)

        # 4.
        # compare their solving time on GearOpt_BO
        func = create_f(model)

        start_time = time.time()
        result_nested_if_else = GearOpt_BO.gearopt_bo(
            func, z3_nested_if_else, z3_features, 
            lower_bounds, upper_bounds, radius_list, 
            5, max_i_A_max=5
        )
        end_time = time.time()
        time_nested_if_else = end_time - start_time

        start_time = time.time()
        result_flatten_imply = GearOpt_BO.gearopt_bo(
            func, z3_flatten_imply, z3_features, 
            lower_bounds, upper_bounds, radius_list, 
            5, max_i_A_max=5
        )
        end_time = time.time()
        time_flatten_imply = end_time - start_time


        print()
        print('Result of nested:', result_nested_if_else, ', time used:', time_nested_if_else)
        print()
        print('Result of flatten:', result_flatten_imply, ', time used:', time_flatten_imply)
        ...
        #     s1 = copy_solver(z3_nested_if_else)
        #     s2 = copy_solver(z3_flatten_imply)

        #     for i in range(dimensions):
        #         s1.add(z3_features[i] == l[i])
        #         s2.add(z3_features[i] == l[i])

        #     s1.check()
        #     s2.check()
        #     sol1 = float(s1.model()[z3.Real('y')].as_fraction())
        #     sol2 = float(s2.model()[z3.Real('y')].as_fraction())
